...
function Login() {
  ...
  const handleSubmit = async (e) => {
    setLoading(true);
    e.preventDefault();
    const data = await loginUser(process.env.BACKEND + '/user/login/', user);
            ...
            <div className="bg-formBox dark:text-mainText/75 flex flex-col gap-3 dark:md:border-0 md:border-2 shadow-lg p-10">
              <div className="hover:cursor-pointer flex flex-col md:flex-row items-center gap-4 md:gap-10">
                ...
                <div className="flex justify-end">
                  <span
                    onClick={logout}
                    className={` font-sans hover:underline text-gray-400`}
                  >
              ...
              >
                ...
                <div className="flex flex-col gap-5 md:gap-7">
                  <div className="flex gap-8 items-center justify-center md:justify-start">
                    <button
                      disabled={loading}
                      className={`font-sans text-xl font-semibold transition-all ${
                        loading
                          ? ''
                          : 'hover:-translate-y-0.5 hover:bg-blue-700'
                      } bg-blue-500 text-white py-2 px-4 rounded focus:outline-none focus:shadow-outline`}
                      type="submit"
                    >
                      Login
                    </button>
